...
class InitDriver:
    ...
    def __init__(self,deviceinfo):
        ...
        self.caps['udid'] = self.udid
        self.driver = webdriver.Remote(f"{self.appiumserver}:{self.port}/wd/hub", self.caps)
    ...
    def find_element(self,ele_info):
        ...
        # value = ele_info['value']
        # timeout = ele_info['timeout']
        # # self.driver.find_element(By.LINK_TEXT,'xxx')
        # if type == 'id':
        #     locator = (By.ID,value)
        # elif type == 'name':
        ...
        try:
            ...
            self.logger.error('查找【{}】使用【{},{}】定位失败,失败原因：{}'.format(name, type, value,e))
            raise Exception('查找【{}】使用【{},{}】定位失败,失败原因：{}'.format(name, type, value,e))
        return element
    ...
    def get_text(self,ele_info):
        element = self.find_element(ele_info)
        self.logger.info('获取元素【{}】的文本是:{}'.format(ele_info['name'],element.text))
        return element.text
    def refresh(self):
        self.driver.refresh()
    # 封装一个截图方法，返回结果是二进制对象
    ...
    def swipe_on_element(self, ele_info, direction, duration=500):
        ...
        if direction.lower() == 'up':
            self.driver.swipe(start_x + w / 2, start_y + 0.8 * h, start_x + w / 2, start_y + 0.2 * h, duration)
        elif direction.lower() == 'down':
            self.driver.swipe(start_x + w / 2, start_y + 0.2 * h, start_x + w / 2, start_y + 0.8 * h, duration)
        elif direction.lower() == 'left':
            self.driver.swipe(start_x + 0.8 * w, start_y + h / 2, start_x + 0.2 * w, start_y + h / 2, duration)
        elif direction.lower() == 'right':
            self.driver.swipe(start_x + 0.2 * w, start_y + h / 2, start_x + 0.8 * w, start_y + h / 2, duration)
        else:
            raise Exception('方向错误')
    # 九宫格手势解锁
    def action_swipe(self, ele_info, pwd):
        ...
        action.press(x=gesture_list[int(pwd[0])].get('x'), y=gesture_list[int(pwd[0])].get('y'))
        for i in range(1, len(pwd)):
            action.move_to(x=gesture_list[int(pwd[i])].get('x'), y=gesture_list[int(pwd[i])].get('y'))
...
class element_click_success(object):
    ...
    def __call__(self, driver):
        ...
        element = driver.find_element(*self.locator) #find_element(By.ID,'xxx')
