...
def init_driver(deviceinfo,worker_id):
    # 三层结构中都需要driver对象，但是我们在conftest里实现driver初始化在传递上存在一定的困难
    # 如何解决？
    # 我们可以采用自定义一个全局性的driver对象来进行传递
    ...
    tpe.submit(AppiumServer().start,port,chromedriver_port) #submit里边放得是你的要在线程里执行的函数，一定注意这个函数不要加()
...
def case_setup_teardown(worker_id):
    ...
    t = threading.Thread(target=shot,args=(GlobalDriver.driver,worker_id))#创建一个线程对象，target表示线程里要执行的方法
    t.start()
    ...
    #     img = Image.open(os.path.join('video', img))
    #     ele_imgs.append(img)
    # setting.DIR_NAME + f'/video/{worker_id}record.gif' 这是拼接生成的gif图名称
